...
class TestPlaceDao:
    ...
    async def categories_fixture(self, session: AsyncSession) -> list[Category]:
        """
        Фикстура для создания тестовых категорий.
        """
        categories = [Category(**data.model_dump()) for data in self.CATEGORIES_DATA]
        session.add_all(categories)
        await session.commit()
        return categories

    @pytest_asyncio.fixture(scope='function')
    async def places_fixture(self, session: AsyncSession, categories_fixture: list[Category]) -> list[Place]:
        """
        Фикстура для создания тестовых заведений.
        """
        places = [Place(**data.model_dump()) for data in self.PLACES_DATA]
        for place in places:
            place.categories = categories_fixture
    ...
    async def test_create_place(self, session: AsyncSession, categories_fixture: list[Category]):
        ...
        place = await PlaceDao.create_place(session, [_.name for _ in categories_fixture], place_data)
        ...
        assert len(place.categories) == len(categories_fixture)
